...
def problem_two(request):
  ...
  for instructor in instructors_before_2010:
    print(f"Instructor Name: {instructor.first_name} {instructor.last_name} Hire Date: {instructor.hire_date}")
...
def problem_five(request):
  nicole = Student.objects.create(first_name ="Nicole", last_name ="Overholt", year=12, gpa=3.5)
  print(f'''Id: {nicole.id}
Full Name: {nicole.first_name} {nicole.last_name}
Year: {nicole.year}
GPA: {nicole.gpa}
''')
...
def problem_six(request):
  ...
  for student in students:
    student_count += 1
  new_student = Student.objects.filter(id=student_count).update(gpa = 4.0)
